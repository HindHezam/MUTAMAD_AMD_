...
(function (global) {
  ...
  const DEFAULT_SETTINGS = {
    ai_webhook_url: "https://hind7777.app.n8n.cloud/webhook/mutamad-analyze", // رابط هند النهائي لمحرك n8n
    webhookSecret: "MUTAMAD_AMD_2026",  // مفتاح الربط الداخلي
    readiness_pass: 70,                 // درجة الجاهزية الدنيا للاعتماد
    readiness_review: 50,               // أقل من ذلك = عالي المخاطر
    sources: ["SAMA"]
  };
  ...
  function getSettings() {
    try {
      ...
      if (!settings.ai_webhook_url && settings.webhookUrl) settings.ai_webhook_url = settings.webhookUrl;
      if (!settings.ai_webhook_url) settings.ai_webhook_url = DEFAULT_SETTINGS.ai_webhook_url;
      if (!settings.webhookSecret) settings.webhookSecret = DEFAULT_SETTINGS.webhookSecret;
      return settings;
    } catch (_) {
  ...
  /* ---------------------- بذرة بيانات احترافية ---------------------- */
  function seedDemoData(force) {
    if (!force && getCampaigns().length) return;

    function analyzeSeed(c) {
      const out = localAnalyze(c);

      c.readiness_score = out.readiness_score;
      c.risk_level = out.risk_level;
      c.status_label = out.status_label;
      c.analysis_summary = out.summary;
      c.findings = out.findings;
      c.recommendations = out.recommendations;
      c.suggested_rewrite = out.suggested_rewrite;
      c.references = out.findings.map(f => f.rule_reference);

      return c;
    }

    function ago(min) {
      return shortDate(new Date(Date.now() - min * 60000).toISOString());
    }

    const base = [
      {
        id: "CMP-2041",
        title: "حملة التمويل الشخصي — موسم الصيف",
        content_type: "إعلان وسائل تواصل",
        product_type: "personal_finance",
        product_type_label: "تمويل شخصي",
        channel: "إنستقرام",
        regulator: "SAMA",
        audience: "موظفو القطاع الخاص",
        objective: "زيادة طلبات التمويل",
        campaign_owner: "ريم العتيبي",
        content_text: "احصل على تمويل فوري بدون رسوم وبموافقة مضمونة للجميع خلال دقيقة.",
        status: "pending_compliance",
        created_at: ago(180),
        updated_at: ago(40),
        timeline: [
          {
            action: "تم إنشاء الحملة كمسودة",
            at: ago(180)
          },
          {
            action: "اكتمل الفحص الذكي للمحتوى",
            at: ago(120)
          },
          {
            action: "أُرسلت الحملة إلى مسؤول الامتثال",
            at: ago(40)
          }
        ]
      },
      {
        id: "CMP-2040",
        title: "بطاقة الإنماء الائتمانية",
        content_type: "صفحة هبوط",
        product_type: "credit_card",
        product_type_label: "بطاقة ائتمانية",
        channel: "صفحة هبوط",
        regulator: "SAMA",
        audience: "العملاء الحاليون",
        objective: "ترقية شريحة البطاقات",
        campaign_owner: "سعد القحطاني",
        content_text: "بطاقة ائتمانية بمزايا أوسع وأقساط مرنة. تطبق الشروط والأحكام. معدل النسبة السنوية APR موضّح في تفاصيل المنتج.",
        status: "approved",
        created_at: ago(1440),
        updated_at: ago(600),
        compliance_decision: "approved",
        compliance_notes: "صياغة منضبطة ومستوفية لمتطلبات الإفصاح.",
        approved_at: ago(600),
        timeline: [
          {
            action: "تم إنشاء الحملة",
            at: ago(1440)
          },
          {
            action: "اكتمل الفحص الذكي للمحتوى",
          ...
          {
            action: "اعتمد مسؤول الامتثال الحملة",
            at: ago(600)
          }
        ]
      },
      {
        id: "CMP-2039",
        title: "عرض تقسيط الأجهزة الذكية",
        content_type: "رسالة نصية",
        product_type: "bnpl",
        product_type_label: "اشترِ الآن وادفع لاحقًا",
        channel: "رسائل SMS",
        regulator: "SAMA",
        audience: "عملاء التجزئة",
        objective: "تفعيل خدمة التقسيط",
        campaign_owner: "نورة الحربي",
        content_text: "قسّط مشترياتك الآن بدون رسوم وبموافقة فورية.",
        status: "needs_revision",
        created_at: ago(2880),
        updated_at: ago(2000),
        compliance_decision: "needs_revision",
        compliance_notes: "يلزم توضيح الرسوم وإزالة وعد الموافقة الفورية قبل النشر.",
        timeline: [
          {
            action: "تم إنشاء الحملة",
            at: ago(2880)
          ...
          {
            action: "أعاد مسؤول الامتثال الحملة للتعديل",
            at: ago(2000)
          }
        ]
      },
      {
        id: "CMP-2038",
        title: "محفظة الإنماء الرقمية",
        content_type: "إعلان داخل التطبيق",
        product_type: "payment",
        product_type_label: "خدمات الدفع",
        channel: "داخل التطبيق",
        regulator: "SAMA",
        audience: "مستخدمو التطبيق",
        objective: "تفعيل المحفظة الرقمية",
        campaign_owner: "خالد المطيري",
        content_text: "ادفع واستقبل أموالك عبر محفظة الإنماء الرقمية بكل سهولة وأمان. تطبق الشروط والأحكام.",
        status: "pending_compliance",
        created_at: ago(300),
        updated_at: ago(90),
        timeline: [
      ...
      {
        id: "CMP-2037",
        title: "حملة التمويل العقاري الميسّر",
        content_type: "إعلان مصوّر",
        product_type: "personal_finance",
        product_type_label: "تمويل عقاري",
        channel: "يوتيوب",
        regulator: "SAMA",
        audience: "الأسر السعودية",
        objective: "التعريف بالتمويل العقاري",
        campaign_owner: "عبدالله الدوسري",
        content_text: "تمويل عقاري بأقساط مدروسة تناسب دخلك. تطبق الشروط والأحكام، ومعدل النسبة السنوية APR موضّح ضمن تفاصيل المنتج.",
        status: "approved",
        created_at: ago(4320),
        updated_at: ago(3600),
        compliance_decision: "approved",
        compliance_notes: "مستوفية لمتطلبات الإفصاح والشفافية.",
        approved_at: ago(3600),
        timeline: [
          {
            action: "تم إنشاء الحملة",
            at: ago(4320)
    ...
    base.forEach(analyzeSeed);
    saveCampaigns(base);

    if (!getReports().length) {
      const approved = base.find(c => c.id === "CMP-2040");

      const rep = buildCampaignReport(approved, {
        manager_note: "الحملة جاهزة للنشر بعد استيفاء متطلبات الإفصاح."
      });
  ...
  // تحميل بيانات أولية احترافية تلقائياً عند أول تشغيل لضمان عمل اللوحات.
  ensureSeed();
})(window);