/* =====================================================================
   مُعتمد | Mu'tamad — طبقة التطبيق المشتركة (المصدر الموحد)
   - مفاتيح التخزين والدوال المساعدة المركزية
   - تنقّل واعٍ بالأدوار
   - عميل تحليل ذكي يستدعي مسار n8n للامتثال
     (webhook: /webhook/mutamad-analyze) مع محرك محلي احتياطي يعيد
     نفس بنية العقد:
       { readiness_score, status, violations_count, summary,
         findings[], recommendations[], suggested_rewrite }
   - منطق موحّد لإحصاءات المنصة وإنشاء التقارير وعقد التقرير النهائي
   ===================================================================== */
(function (global) {
  "use strict";
  ...
  function getCampaigns() { return read(KEYS.campaigns, []); }
  function saveCampaigns(list) { write(KEYS.campaigns, list); }
  function getReports() { return read(KEYS.reports, []); }
  function saveReports(list) { write(KEYS.reports, list); }
  ...
  function getRole() { return localStorage.getItem(KEYS.role) || "marketing"; }
  function setRole(r) { localStorage.setItem(KEYS.role, r); }
  function logout() {
    localStorage.removeItem(KEYS.role);
    location.href = "login.html";
  }

  /* ---------------------- تسميات الحالة والمخاطر ---------------------- */
  ...
  function riskLabel(risk) {
    const labels = {
      ...
      "منخفض": "منخفض", "متوسط": "متوسط", "عالي": "عالي"
    };
  ...
  function pushTimeline(campaign, action) {
    ...
    campaign.timeline.push({ action: action, at: shortDate(nowISO()) });
  }

  /* ---------------------- إحصاءات المنصة (موحّدة) ---------------------- */
  function collectStats(campaigns) {
    const list = Array.isArray(campaigns) ? campaigns : getCampaigns();
    const total = list.length;
    const by = function (fn) { return list.filter(fn).length; };
    ...
    const pct = function (part, base) { return base ? Math.round((part / base) * 100) : 0; };

    return {
      total, pending, revision, approved, rejected, draft,
  ...
  /* ---------------------- بناء التقارير (عقد موحّد) ----------------------
     يُنتج كائن تقرير يتوافق مع ما تعرضه صفحة التقرير النهائي:
       { id, type, title, scope, created_at, stamp, stamp_text,
         summary_title, summary_text, summary, recommendation,
         manager_notes, counts{}, average_readiness, risk_rate,
         approval_rate, revision_rate, ...campaign fields }            */
  function buildGeneralReport(opts) {
    ...
    return {
      ...
      counts: {
        total: s.total, pending: s.pending, revision: s.revision,
        approved: s.approved, rejected: s.rejected, highRisk: s.highRisk
      },
  ...
  function buildCampaignReport(campaign, opts) {
    ...
    // التقرير المرتبط بحملة يُعرض كذلك ضمن بنية العدّادات لتظهر مؤشراته
    const counts = {
      total: 1,
  ...
  function setCurrentReport(id) {
    ...
    if (r) localStorage.setItem(KEYS.currentReportObj, JSON.stringify(r));
  }
  ...
  /* ---------------------- محرك احتياطي محلي ----------------------
     يحاكي مسار n8n رباعي الوكلاء: استخراج العبارات -> مطابقة قواعد
     SAMA -> إعادة صياغة -> احتساب الدرجة. معرّفات القواعد مطابقة. */
  const RULES = [
    {
      id: "SAMA_ADV_001", type: "guaranteed_approval", risk: "عالي", impact: 30,
      patterns: ["موافقة مضمونة", "قبول مضمون", "مضمون", "للجميع", "موافقة فورية للجميع", "100% موافقة", "بدون رفض"],
      category: "وعد_مطلق",
    ...
    {
      id: "SAMA_ADV_014", type: "no_fees_claim", risk: "عالي", impact: 22,
      patterns: ["بدون رسوم", "بلا رسوم", "بدون أي رسوم", "مجاناً", "مجانا", "بدون فوائد"],
      category: "ادعاء_رسوم",
    ...
    {
      id: "SAMA_101", type: "missing_apr_disclosure", risk: "عالي", impact: 25,
      patterns: [],
      category: "إفصاح_ناقص",
    ...
    {
      id: "SAMA_ADV_022", type: "time_promise", risk: "متوسط", impact: 12,
      patterns: ["فوري", "خلال دقيقة", "خلال دقائق", "في الحال", "لحظي", "تمويل فوري"],
      category: "سرعة",
    ...
    {
      id: "SAMA_ADV_031", type: "missing_terms", risk: "متوسط", impact: 10,
      patterns: [],
      category: "إفصاح_ناقص",
    ...
    {
      id: "SAMA_PAY_068", type: "unclear_payment_ad", risk: "متوسط", impact: 12,
      patterns: ["محفظة", "خدمة دفع", "دفع إلكتروني"],
      category: "إفصاح_ناقص",
  ...
  function localAnalyze(input) {
    ...
    RULES.forEach(rule => {
      ...
      if (matched) {
        findings.push({
          claim_text: matched, category: rule.category, risk_level: rule.risk,
          violation_id: rule.id, violation_type: rule.type,
          rule_reference: rule.reference, rule_source: "SAMA",
          score_impact: rule.impact, confidence: "مؤكد", reason: rule.reason
        });
        recommendations.push(rule.fix);
      }
    });

    const isFinancing = ["personal_finance", "credit_card", "bnpl", "installment"].includes(product) ||
    ...
    if (isFinancing && !mentionsApr) {
      ...
      findings.push({
        claim_text: "غياب معدل النسبة السنوية APR", category: "إفصاح_ناقص",
        risk_level: "عالي", violation_id: r.id, violation_type: r.type,
        rule_reference: r.reference, rule_source: "SAMA", score_impact: r.impact,
        confidence: "مؤكد", reason: r.reason
      });
    ...
    if (!mentionsTerms && text.length > 0) {
      ...
      findings.push({
        claim_text: "غياب الإشارة للشروط والأحكام", category: "إفصاح_ناقص",
        risk_level: "متوسط", violation_id: r.id, violation_type: r.type,
        rule_reference: r.reference, rule_source: "SAMA", score_impact: r.impact,
        confidence: "محتمل", reason: r.reason
      });
      recommendations.push(r.fix);
    }
    ...
    let status;
    if (deduped.length === 0) status = "منخفض";
    else if (score >= 70) status = "يحتاج مراجعة";
    else status = "عالي المخاطر";
    ...
    return {
      readiness_score: score, status: status, risk_level: statusToRisk(status),
      status_label: status, violations_count: deduped.length, summary: summary,
      findings: deduped, recommendations: Array.from(new Set(recommendations)),
      suggested_rewrite: rewrite, total_score_impact: totalImpact, engine: "local"
    };
  }
  ...
  function buildRewrite(text, findings) {
    ...
    findings.forEach(f => {
      ...
      if (rule && rule.patterns.length) {
        if (out.includes(f.claim_text)) out = out.replace(f.claim_text, rule.fix);
        rule.patterns.forEach(p => {
          if (p !== f.claim_text && out.includes(p)) out = out.replace(p, "");
        });
      }
    });
    out = out.replace(/\s{2,}/g, " ").replace(/\s+([،.])/g, "$1").trim();
    if (!/شروط|الأحكام/.test(out)) out += " — تطبق الشروط والأحكام.";
  ...
  async function analyze(input) {
    ...
    if (s.ai_webhook_url) {
      ...
      if (!res.ok) throw new Error("AI webhook returned " + res.status);
      ...
      out.violations_count = out.violations_count != null
        ? out.violations_count : (out.findings ? out.findings.length : 0);
  ...
  /* ---------------------- تهيئة التخزين بدون بيانات جاهزة ----------------------
     لا يتم إنشاء أي حملات أو تقارير تلقائيًا.
     يتم فقط تنظيف السجلات القديمة التي كانت ضمن نسخة العرض السابقة إن وجدت. */
  function cleanupLegacySampleData() {
    try {
      const sampleIds = new Set(["CMP-2041", "CMP-2040", "CMP-2039", "CMP-2038", "CMP-2037"]);

      const campaigns = getCampaigns();
      const cleanedCampaigns = campaigns.filter(c => !sampleIds.has(c.id));
      ...
      const cleanedReports = reports.filter(r => {
        if (r && r.campaign_id && sampleIds.has(r.campaign_id)) return false;
        if (r && r.campaign_snapshot && sampleIds.has(r.campaign_snapshot.id)) return false;
        return true;
      });
  ...
  function initializeStorage() {
    cleanupLegacySampleData();
  }

  global.Mutamad = {
    KEYS, ROLE_LABELS, RULES,
    getSettings, saveSettings,
    read, write,
    getCampaigns, saveCampaigns, getReports, saveReports,
    uid, nowISO, shortDate,
    getRole, setRole, logout,
    statusLabel, riskLabel, statusClass, riskClass,
    pushTimeline, collectStats,
    buildGeneralReport, buildCampaignReport, commitReport,
    setCurrentReport, getCurrentReport, deleteReport,
    analyze, localAnalyze, statusToRisk,
    escapeHtml,
    cleanupLegacySampleData, initializeStorage
  };

  // لا يتم إنشاء بيانات جاهزة؛ يتم تنظيف السجلات القديمة فقط عند التشغيل.
  initializeStorage();
})(window);
